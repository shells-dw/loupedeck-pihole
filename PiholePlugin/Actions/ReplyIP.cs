namespace Loupedeck.PiholePlugin.Actions
{
    ...
    internal class ReplyIp : PluginDynamicCommand
    {
        // Assign variables

        private PiholePlugin _plugin;
        // build action
        public ReplyIp() : base() => this.AddParameter("replyip", "Display IP reply", groupName: "Display");
        ...
        protected override BitmapImage GetCommandImage(String actionParameter, PluginImageSize imageSize)
        {
            ...
            using (var bitmapBuilder = new BitmapBuilder(imageSize))
            {
                ...
                bitmapBuilder.DrawText(String.Format("{0:n0}", Int32.Parse(Globals.PiDump.reply_ip)), x: 40, y: 5, width: 0, height: 40, fontSize: 15, color: BitmapColor.White);
                bitmapBuilder.DrawText("IP\nreply", x: 5, y: 35, width: 70, height: 40, fontSize: 12, color: BitmapColor.White);
